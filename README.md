...
The app and worker load `.env.development` and then `.env`; variables already set by the service take precedence. Database host, port, name and user, R2 account and bucket, job schedule, dry run and the job enabled flag are edited in the UI. Jobs can also be enabled or disabled directly from the Jobs list; that action refreshes the managed cron file. Keep `DB_PASSWORD`, `ARCHIVE_R2_ACCESS_KEY` and `ARCHIVE_R2_SECRET_KEY` in the environment file. The environment password applies only when the connection's database name and user match `DB_NAME` and `DB_USER`; R2 keys apply only when a job's account and bucket match `ARCHIVE_R2_ACCOUNT_ID` and `ARCHIVE_R2_BUCKET`. For other connections or jobs, use `VAULTLINE_CONNECTION_<id>_PASSWORD` and `VAULTLINE_JOB_<id>_R2_ACCESS_KEY`/`VAULTLINE_JOB_<id>_R2_SECRET_KEY`. Existing SQLite secrets remain a compatibility fallback until removed.
...
Use Gunicorn behind Nginx, keep `VAULTLINE_SECRET` in an environment file, and run `worker.py` plus cron under a restricted service account (or root, if that matches how the rest of the host is run — `VAULTLINE_CRON_USER` defaults to `root` since the managed jobs run native dump tools and delete rows, and a non-root Gunicorn account needs explicit write access to `/etc/cron.d/vaultline` before **Settings → Sync now** will work). Install the OS-level backup tools before disabling dry run. The worker verifies uploaded object size before any archive/retention delete is committed. SQL Server jobs use a compressed logical table export because native `.bak` files must be written on the SQL Server host.
...
# production, once this app is running on the target host
